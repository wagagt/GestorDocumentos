        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.adjunto.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.adjuntos.update", [$adjunto->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('caso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="caso_id" id="caso_id" required>
                                @foreach($casos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('caso_id') ? old('caso_id') : $adjunto->caso->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('caso'))
                        ...
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $adjunto->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('tipo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="tipo" id="tipo">
                                ...
                                @foreach(App\Models\Adjunto::TIPO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo', $adjunto->tipo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('tipo'))
