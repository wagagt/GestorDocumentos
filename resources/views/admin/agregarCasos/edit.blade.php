                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.agregar-casos.update", [$agregarCaso->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $agregarCaso->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('descripcion') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="descripcion" id="descripcion" required>{{ old('descripcion', $agregarCaso->descripcion) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('flujo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="flujo_id" id="flujo_id" required>
                                @foreach($flujos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('flujo_id') ? old('flujo_id') : $agregarCaso->flujo->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('flujo'))
                        ...
                        <div class="form-group {{ $errors->has('fecha_creacion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_creacion" id="fecha_creacion" value="{{ old('fecha_creacion', $agregarCaso->fecha_creacion) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('encargado') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="encargado_id" id="encargado_id" required>
                                @foreach($encargados as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('encargado_id') ? old('encargado_id') : $agregarCaso->encargado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
