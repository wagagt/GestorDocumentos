                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.caso-pasos.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('paso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="paso_id" id="paso_id" required>
                                @foreach($pasos as $id => $entry)
                                    <option value="{{ $id }}" {{ old('paso_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('paso'))
                        ...
                        <div class="form-group {{ $errors->has('status_actual') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status_actual" id="status_actual" required>
                                ...
                                @foreach(App\Models\CasoPaso::STATUS_ACTUAL_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status_actual', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('fecha_inicio') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio') }}" required>
                        ...
                        <div class="form-group {{ $errors->has('fecha_fin') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin') }}" required>
