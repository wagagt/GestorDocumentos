                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.comentarios.update", [$comentario->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('caso') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="caso_id" id="caso_id" required>
                                @foreach($casos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('caso_id') ? old('caso_id') : $comentario->caso->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('caso'))
                        ...
                        <div class="form-group {{ $errors->has('nombre') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="nombre" id="nombre" value="{{ old('nombre', $comentario->nombre) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('propietario') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="propietario_id" id="propietario_id" required>
                                @foreach($propietarios as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('propietario_id') ? old('propietario_id') : $comentario->propietario->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('fecha') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="fecha" id="fecha" value="{{ old('fecha', $comentario->fecha) }}" required>
                            @if($errors->has('fecha'))
                        ...
                        <div class="form-group {{ $errors->has('comentario') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="comentario" id="comentario" value="{{ old('comentario', $comentario->comentario) }}">
