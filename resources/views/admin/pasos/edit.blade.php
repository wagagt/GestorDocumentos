                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.pasos.update", [$paso->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('flujo') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="flujo_id" id="flujo_id" required>
                                @foreach($flujos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('flujo_id') ? old('flujo_id') : $paso->flujo->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('flujo'))
                        ...
                        <div class="form-group {{ $errors->has('orden') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="orden" id="orden" value="{{ old('orden', $paso->orden) }}" required>
                            @if($errors->has('orden'))
                        ...
                        <div class="form-group {{ $errors->has('descripcion') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="descripcion" id="descripcion" required>{{ old('descripcion', $paso->descripcion) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('requisitos_aceptacion') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="requisitos_aceptacion" id="requisitos_aceptacion">{{ old('requisitos_aceptacion', $paso->requisitos_aceptacion) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('duracion_aproximada') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="duracion_aproximada" id="duracion_aproximada" value="{{ old('duracion_aproximada', $paso->duracion_aproximada) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('empleado') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="empleado_id" id="empleado_id" required>
                                @foreach($empleados as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('empleado_id') ? old('empleado_id') : $paso->empleado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('requisitos_liberacion') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="requisitos_liberacion" id="requisitos_liberacion" value="{{ old('requisitos_liberacion', $paso->requisitos_liberacion) }}">
                        ...
                        <div class="form-group {{ $errors->has('estado') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="estado" id="estado" required>
                                ...
                                @foreach(App\Models\Paso::ESTADO_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('estado', $paso->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
